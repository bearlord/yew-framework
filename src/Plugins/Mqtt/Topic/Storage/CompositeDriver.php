 ...
 *
 * Local copy of Yew\Plugins\Topic\Storage\CompositeDriver so the Mqtt plugin
...
class CompositeDriver implements DriverInterface
{
    ...
    public function addSubscription(string $topic, string $clientId): bool
    {
        $success = true;
        foreach ($this->drivers as $driver) {
            if (!$driver->addSubscription($topic, $clientId)) {
                $success = false;
            }
        }
        return $success;
    ...
    public function removeSubscription(string $topic, string $clientId): bool
    {
        $success = true;
        foreach ($this->drivers as $driver) {
            if (!$driver->removeSubscription($topic, $clientId)) {
                $success = false;
            }
        }
        return $success;
     ...
     * @return array|null List of subscriber client_ids
     */
    ...
    /**
     * Get subscriptions for a client_id from the persistent driver
     *
     * @param string $clientId The subscriber client_id
    ...
    {
        return $this->persistent()->getSubscriptions($clientId);
    }
