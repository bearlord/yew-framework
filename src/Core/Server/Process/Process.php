...
abstract class Process
{
    ...
    /**
     * Bytes read from the IPC pipe per recv() call.
     */
    const IPC_RECV_CHUNK_SIZE = 65536;

    /**
     * Max bytes for the in-memory IPC reassembly buffer.
     */
    const IPC_MAX_BUFFER_SIZE = 16 * 1024 * 1024;

    /**
     * Max accepted IPC frame length (larger is a protocol error).
     */
    const IPC_MAX_FRAME_SIZE = 16 * 1024 * 1024;

    /**
     * Min / max number of IPC consumer coroutines.
     */
    const IPC_CONSUMER_MIN = 4;
    const IPC_CONSUMER_MAX = 16;

    /**
     * Mailbox length high-water mark for the elastic monitor.
     */
    const IPC_SCALE_UP_LEN = 256;

    /**
     * Idle seconds before a dynamic consumer self-exits.
     */
