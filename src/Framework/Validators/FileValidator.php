...
class FileValidator extends Validator
{
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = Yew::t('yew', 'File upload failed.');
        }
        if ($this->uploadRequired === null) {
            $this->uploadRequired = Yew::t('yew', 'Please upload a file.');
        }
        if ($this->tooMany === null) {
            $this->tooMany = Yew::t('yew', 'You can upload at most {limit, number} {limit, plural, one{file} other{files}}.');
        }
        if ($this->tooFew === null) {
            $this->tooFew = Yew::t('yew', 'You should upload at least {limit, number} {limit, plural, one{file} other{files}}.');
        }
        if ($this->wrongExtension === null) {
            $this->wrongExtension = Yew::t('yew', 'Only files with these extensions are allowed: {extensions}.');
        }
        if ($this->tooBig === null) {
            $this->tooBig = Yew::t('yew', 'The file "{file}" is too big. Its size cannot exceed {formattedLimit}.');
        }
        if ($this->tooSmall === null) {
            $this->tooSmall = Yew::t('yew', 'The file "{file}" is too small. Its size cannot be smaller than {formattedLimit}.');
        }
        ...
        if ($this->wrongMimeType === null) {
            $this->wrongMimeType = Yew::t('yew', 'Only files with these MIME types are allowed: {mimeTypes}.');
        }
    ...
    protected function validateValue($value)
    {
        ...
        switch ($value->error) {
                ...
                if ($this->maxSize !== null && $value->size > $this->getSizeLimit()) {
                    ...
                    return [
                        ...
                        [
                            'file' => $value->name,
                            'limit' => $this->minSize,
                            'formattedLimit' => Yew::$app->formatter->asShortSize($this->minSize),
                        ],
                        $this->getValidCode()
                    ];
            ...
            case UPLOAD_ERR_PARTIAL:
                Yew::warning('File was only partially uploaded: ' . $value->name, __METHOD__);
                break;
            case UPLOAD_ERR_NO_TMP_DIR:
                Yew::warning('Missing the temporary folder to store the uploaded file: ' . $value->name, __METHOD__);
                break;
            case UPLOAD_ERR_CANT_WRITE:
                Yew::warning('Failed to write the uploaded file to disk: ' . $value->name, __METHOD__);
                break;
            case UPLOAD_ERR_EXTENSION:
                Yew::warning('File upload was stopped by some PHP extension: ' . $value->name, __METHOD__);
                break;
            default:
                break;
        }
    ...
    public function getSizeLimit()
    {
        ...
        if ($postLimit > 0 && $postLimit < $limit) {
            Yew::warning('PHP.ini\'s \'post_max_size\' is less than \'upload_max_filesize\'.', __METHOD__);
            $limit = $postLimit;
        }
