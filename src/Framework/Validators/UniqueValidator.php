...
use Yew\Framework\Db\ActiveRecordInterface;
...
class UniqueValidator extends Validator
{
    ...
    public $targetClass;
    /**
     * @var string|array the name of the [[\Yew\Framework\Db\ActiveRecord|ActiveRecord]] attribute that should be used to
    ...
    public function init()
    {
        ...
        if (is_array($this->targetAttribute) && count($this->targetAttribute) > 1) {
            ...
            if ($this->comboNotUnique === null) {
                $this->message = Yew::t('yew', 'The combination {values} of {attributes} has already been taken.');
            } else {
                $this->message = $this->comboNotUnique;
            }
        } else {
            $this->message = Yew::t('yew', '{attribute} "{value}" has already been taken.');
        }
    }
    ...
    public function validateAttribute($model, $attribute)
    {
        ...
        foreach ($rawConditions as $key => $value) {
            if (is_array($value)) {
                $this->addError($model, $attribute, Yew::t('yew', '{attribute} is invalid.'));
                return;
            }
     ...
     * @param array $conditions conditions, compatible with [[\Yew\Framework\Db\Query::where()|Query::where()]] key-value format.
     * @param Model $model the data model to be validated
     *
    ...
    {
        /** @var ActiveRecordInterface|\Yew\Framework\Base\BaseObject $targetClass $query */
        ...
        if (!$model instanceof ActiveRecordInterface || $model->getIsNewRecord() || $model->className() !== $targetClass::className()) {
            ...
            if ($query instanceof \Yew\Framework\Db\ActiveQuery) {
     ...
     *
     * @param string|array $targetAttribute the name of the [[\Yew\Framework\Db\ActiveRecord|ActiveRecord]] attribute that
