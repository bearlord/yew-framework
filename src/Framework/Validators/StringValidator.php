...
class StringValidator extends Validator
{
     ...
     * If this property is not set, [[\Yew\Framework\Base\Application::charset]] will be used.
     */
    public $encoding;
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = Yew::t('yew', '{attribute} must be a string.');
        }
        if ($this->min !== null && $this->tooShort === null) {
            $this->tooShort = Yew::t('yew', '{attribute} should contain at least {min, number} {min, plural, one{character} other{characters}}.');
        }
        if ($this->max !== null && $this->tooLong === null) {
            $this->tooLong = Yew::t('yew', '{attribute} should contain at most {max, number} {max, plural, one{character} other{characters}}.');
        }
        if ($this->length !== null && $this->notEqual === null) {
            $this->notEqual = Yew::t('yew', '{attribute} should contain {length, number} {length, plural, one{character} other{characters}}.');
        }
    }
    ...
    protected function validateValue($value)
    {
        ...
        if ($this->min !== null && $length < $this->min) {
            $this->setValidCode(1900001);
            return [$this->tooShort, ['min' => $this->min], $this->getValidCode()];
        }
        if ($this->max !== null && $length > $this->max) {
            $this->setValidCode(1900002);
            return [$this->tooLong, ['max' => $this->max], $this->getValidCode()];
        }
        if ($this->length !== null && $length !== $this->length) {
            $this->setValidCode(1900003);
            return [$this->notEqual, ['length' => $this->length], $this->getValidCode()];
        }
