 ...
 *
 * AssetManager is configured as an application component in [[\Yew\Framework\Web\Application]] by default.
 * You can access that instance via `Yew::$app->assetManager`.
 *
...
class AssetManager extends Component
{
     ...
     *
     * This is passed as a parameter `beforeCopy` to [[\Yew\Framework\Helpers\FileHelper::copyDirectory()]].
     */
    public $beforeCopy;
    /**
     ...
     * for [[beforeCopy]].
     * This is passed as a parameter `afterCopy` to [[\Yew\Framework\Helpers\FileHelper::copyDirectory()]].
     */
    public $afterCopy;
    /**
    ...
    {
        parent::init();
        $this->basePath = Yew::getAlias($this->basePath);
        if (!is_dir($this->basePath)) {
            throw new InvalidConfigException("The directory does not exist: {$this->basePath}");
        }

        $this->basePath = realpath($this->basePath);
        $this->baseUrl = rtrim(Yew::getAlias($this->baseUrl), '/');
    }
    ...
    protected function loadBundle($name, $config = [], $publish = true)
    {
        ...
        $bundle = Yew::createObject($config);
        if ($publish) {
            $bundle->publish($this);
        }
    ...
    protected function loadDummyBundle($name)
    {
        if (!isset($this->_dummyBundles[$name])) {
            $bundle = Yew::createObject(['class' => $name]);
    ...
    public function getAssetUrl($bundle, $asset)
    {
        if (($actualAsset = $this->resolveAsset($bundle, $asset)) !== false) {
            if (strncmp($actualAsset, '@web/', 5) === 0) {
                ...
                $basePath = Yew::getAlias('@webroot');
                $baseUrl = Yew::getAlias('@web');
            } else {
                $asset = Yew::getAlias($actualAsset);
    ...
    protected function resolveAsset($bundle, $asset)
    {
        ...
        $n = mb_strlen($asset, Yew::$app->charset);
        foreach ($this->assetMap as $from => $to) {
            $n2 = mb_strlen($from, Yew::$app->charset);
    ...
    protected function hash($path)
    {
        ...
        return sprintf('%x', crc32($path . Yew::getVersion() . '|' . $this->linkAssets));
    }
}
