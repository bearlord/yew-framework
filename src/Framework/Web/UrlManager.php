...
class UrlManager extends Component
{
    ...
    public function init()
    {
        ...
        if ($this->normalizer !== false) {
            $this->normalizer = Yew::createObject($this->normalizer);
        ...
        if (is_string($this->cache)) {
            $this->cache = Yew::$app->get($this->cache, false);
        }
        if (empty($this->rules)) {
            return;
        }
    ...
    protected function buildRules($ruleDeclarations)
    {
        ...
        foreach ($ruleDeclarations as $key => $rule) {
            ...
            if (is_array($rule)) {
                $rule = Yew::createObject(array_merge($this->ruleConfig, $rule));
            }
    ...
    public function parseRequest($request)
    {
        if ($this->enablePrettyUrl) {
            ...
            Yew::debug('No matching URL rules. Using default URL parsing logic.', __METHOD__);
        ...
        Yew::debug('Pretty URL not enabled. Using default URL parsing logic.', __METHOD__);
     ...
     *
     * Note that unlike [[\Yew\Framework\Helpers\Url::toRoute()]], this method always treats the given route
    ...
    {
        $this->_baseUrl = $value === null ? null : rtrim(Yew::getAlias($value), '/');
    }
