...
use Yew\Framework\Rbac\CheckAccessInterface;
 ...
 *
 * User is configured as an application component in [[\Yew\Framework\Web\Application]] by default.
 * You can access that instance via `Yew::$app->user`.
 *
...
class User extends Component
{
    ...
    public function init()
    {
        ...
        if (!empty($this->accessChecker) && is_string($this->accessChecker)) {
            $this->accessChecker = Yew::createObject($this->accessChecker);
        }
        $this->session = Yew::$app->getSession();
    ...
    public function login(IdentityInterface $identity, $duration = 0)
    {
        if ($this->beforeLogin($identity, false, $duration)) {
            ...
            $id = $identity->getId();
            $ip = Yew::$app->getRequest()->getServer('remote_addr');
    ...
    public function loginByCookie()
    {
        ...
        if (isset($data['identity'], $data['duration'])) {
            ...
            if ($this->beforeLogin($identity, true, $duration)) {
                ...
                $id = $identity->getId();
                $ip = Yew::$app->getRequest()->getServer(Request::SERVER_REMOTE_ADDR);
                Yew::info("User '$id' logged in from $ip via cookie.", __METHOD__);
    ...
    public function logout($destroySession = true)
    {
        ...
        if ($identity !== null && $this->beforeLogout($identity)) {
            ...
            $id = $identity->getId();
            $ip = Yew::$app->getRequest()->getServer(Request::SERVER_REMOTE_ADDR);
            Yew::info("User '$id' logged out from $ip.", __METHOD__);
            if ($destroySession && $this->enableSession) {
                Yew::$app->getSession()->destroy();
            }
    ...
    {
        $url = Yew::$app->getSession()->get($this->returnUrlParam, $defaultUrl);
    ...
    {
        Yew::$app->getSession()->set($this->returnUrlParam, $url);
    }
    ...
    public function loginRequired($checkAjax = true, $checkAcceptHeader = true)
    {
        ...
        if ($this->loginUrl !== null && $canRedirect) {
            ...
            if ($loginUrl[0] !== Yew::$app->requestedRoute) {
    ...
    protected function renewIdentityCookie()
    {
        ...
        $cookieParams = Yew::$app->getRequest()->getCookieParams();
        ...
        if ($value !== null) {
            ...
            if (is_array($data) && isset($data[2])) {
                ...
                Yew::$app->getResponse()->getCookies()->add($cookie);
            }
        }
    }
    ...
    {
        $validationKey = Yew::$app->cookieValidationKey;
        ...
        $hashData = Yew::$app->getSecurity()->hashData(serialize([$this->identityCookie['name'], $cookieValue]), $validationKey);

        $cookie = new Cookie($this->identityCookie['name'], $hashData, time() + $duration);
        Yew::$app->getResponse()->withCookie($cookie);
    }
    ...
    protected function getIdentityAndDurationFromCookie()
    {
        ...
        $validateData = Yew::$app->getSecurity()->validateData($value, Yew::$app->cookieValidationKey);
        if (!$validateData) {
            return null;
        }
        ...
        if (is_array($data) && count($data) == 3) {
            ...
            if ($identity !== null) {
                if (!$identity instanceof IdentityInterface) {
                    ...
                    Yew::warning("Invalid auth key attempted for user '$id': $authKey", __METHOD__);
                } else {
    ...
    protected function removeIdentityCookie()
    {
        ...
        if (!empty($cookieParams)) {
            foreach ($cookieParams as $k => $v) {
                Yew::$app->getResponse()->withCookie(new Cookie($k, $v));
            }
        }
     ...
     */
    public function can(string $permissionName, array $params = [], bool $allowCaching = true)
    {
    ...
    {
        $acceptableTypes = Yew::$app->getRequest()->getHeader(Request::HEADER_ACCEPT);
