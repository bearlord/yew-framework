 ...
 * @since 2.0
 */
class Transaction extends \Yew\Framework\Base\BaseObject
{
    /**
    ...
    public function begin($isolationLevel = null)
    {
        ...
        if ($this->_level === 0) {
            ...
            Yew::debug('Begin transaction' . ($isolationLevel ? ' with isolation level ' . $isolationLevel : ''), __METHOD__);
        ...
        if ($schema->supportsSavepoint()) {
            Yew::debug('Set savepoint ' . $this->_level, __METHOD__);
            $schema->createSavepoint('LEVEL' . $this->_level);
        } else {
            Yew::info('Transaction not started: nested transaction not supported', __METHOD__);
    ...
    public function commit()
    {
        ...
        $schema = $this->db->getSchema();
        if ($schema->supportsSavepoint()) {
            Yew::debug('Release savepoint ' . $this->_level, __METHOD__);
            $schema->releaseSavepoint('LEVEL' . $this->_level);
        } else {
            Yew::info('Transaction not committed: nested transaction not supported', __METHOD__);
        }
    }
    ...
    public function rollBack()
    {
        ...
        $schema = $this->db->getSchema();
        if ($schema->supportsSavepoint()) {
            Yew::debug('Roll back to savepoint ' . $this->_level, __METHOD__);
            $schema->rollBackSavepoint('LEVEL' . $this->_level);
        } else {
            Yew::info('Transaction not rolled back: nested transaction not supported', __METHOD__);
        }
    }
    ...
    public function setIsolationLevel($level)
    {
        ...
        Yew::debug('Setting transaction isolation level to ' . $level, __METHOD__);
