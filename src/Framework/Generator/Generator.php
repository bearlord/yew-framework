...
abstract class Generator extends Model
{
    ...
    /**
     * @var bool whether the strings will be generated using `Yew::t()` or normal strings.
     */
    public $enableI18N = false;
    /**
     * @var string the message category used by `Yew::t()` when `$enableI18N` is `true`.
     * Defaults to `app`.
     */
     ...
     * Please refer to [[\Yew\Framework\Generator\Model\Generator::generate()]] as an example
    ...
    public function init()
    {
        ...
        foreach ($this->templates as $i => $template) {
            $this->templates[$i] = Yew::getAlias($template);
        }
    }
    ...
    {
        return [
            'enableI18N' => 'This indicates whether the generator should generate strings using <code>Yew::t()</code> method.
                Set this to <code>true</code> if you are planning to make your application translatable.',
            'messageCategory' => 'This is the category used by <code>Yew::t()</code> in case you enable I18N.',
        ];
    }
    ...
    {
        return Yew::$app->getRuntimePath() . '/gii-' . Yew::getVersion() . '/' . str_replace('\\', '-', get_class($this)) . '.json';
    }
    ...
    public function validateTemplate()
    {
        ...
        if (!isset($templates[$this->template])) {
            ...
            foreach ($this->requiredTemplates() as $template) {
                if (!is_file(Yew::getAlias($templatePath . '/' . $template))) {
    ...
    public function validateNewClass($attribute, $params)
    {
        ...
        if (($pos = strrpos($class, '\\')) === false) {
            ...
            $path = Yew::getAlias('@' . str_replace('\\', '/', $ns), false);
            if ($path === false) {
     ...
     * @param array $placeholders the placeholders to use by `Yew::t()`
     * @return string
     */
    public function generateString($string = '', $placeholders = []): string
    {
        ...
        if ($this->enableI18N) {
            ...
            $str = "Yew::t('" . $this->messageCategory . "', '" . $string . "'" . $ph . ")";
        } else {
