 ...
 * not be validated, i.e. `created_at` and `updated_at` should not appear in the [[\Yew\Framework\Base\Model::rules()|rules()]] method of the model.
 *
 ...
 * ```
 *
 * In case you use an [[\Yew\Framework\Db\Expression]] object as in the example above, the attribute will not hold the timestamp value, but
 * the Expression object itself after the record has been saved. If you need the value from DB afterwards you should call
 * the [[\Yew\Framework\Db\ActiveRecord::refresh()|refresh()]] method of the record.
 *
