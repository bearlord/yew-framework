...
use Yew\Framework\Exception\ViewNotFoundException;
use Yew\Yew;
use Yew\Framework\Helpers\FileHelper;
use Yew\Framework\Widgets\Block;
use Yew\Framework\Widgets\ContentDecorator;
use Yew\Framework\Widgets\FragmentCache;
...
class View extends Component implements DynamicContentAwareInterface
{
    ...
    protected function findViewFile(string $view, ?object $context = null): string
    {
        if (strncmp($view, '@', 1) === 0) {
            // e.g. "@app/views/main"
            $file = Yew::getAlias($view);
        } elseif (strncmp($view, '//', 2) === 0) {
            // e.g. "//layouts/main"
            $file = Yew::$app->getViewPath() . DIRECTORY_SEPARATOR . ltrim($view, '/');
    ...
    {
        $viewFile = $requestedFile = Yew::getAlias($viewFile);
     ...
     */
    public function afterRender(string $viewFile, array $params, string &$output)
    {
     ...
     * @param array|null $params the variables (name => value) to be extracted and made available in the decorative view.
     ...
     * @param array|null $properties initial property values for [[FragmentCache]]
