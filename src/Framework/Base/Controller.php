...
class Controller extends Component implements ViewContextInterface
{
     ...
     * ```
     *
     * [[\Yew::createObject()]] will be used later to create the requested action
    ...
    public function runAction(string $id, ?array $params = [])
    {
        ...
        Yew::debug('Route to run: ' . $action->getUniqueId(), __METHOD__);
    ...
    public function run(string $route, ?array $params = [])
    {
        ...
        return Yew::$app->runAction(ltrim($route, '/'), $params);
    }
    ...
    {
        $this->_viewPath = Yew::getAlias($path);
    }
    ...
    public function findLayoutFile(View $view): ?string
    {
        ...
        if (strncmp($layout, '@', 1) === 0) {
            $file = Yew::getAlias($layout);
        } elseif (strncmp($layout, '/', 1) === 0) {
            $file = Yew::$app->getLayoutPath() . DIRECTORY_SEPARATOR . substr($layout, 1);
        } else {
