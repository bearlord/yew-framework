...
{
    /**
     * @var \Yew\Framework\Web\UrlManager URL manager to use for creating URLs
     * @since 2.0.8
     */
     ...
     *
     * This method will use [[\Yew\Framework\Web\UrlManager]] to create a URL.
     *
     ...
     *
     * - If the route is an empty string, the current [[\Yew\Framework\Web\Controller::route|route]] will be used;
     * - If the route contains no slashes at all (e.g. `index`), it is considered to be an action ID
     *   of the current controller and will be prepended with [[\Yew\Framework\Web\Controller::uniqueId]];
     * - If the route has no leading slash (e.g. `site/index`), it is considered to be a route relative
     *   to the current module and will be prepended with the module's [[\Yew\Framework\Base\Module::uniqueId|uniqueId]].
     *
     ...
     * [[\Yew\Framework\Web\UrlManager::$hostInfo]]) will be returned. If `$url` is already an absolute URL, its scheme
     ...
     * later by [[previous()]]. If not set, [[\Yew\Framework\Web\User::setReturnUrl()]] will be used with passed URL.
     * @see previous()
     * @see \Yew\Framework\Web\User::setReturnUrl()
     */
     ...
     * If not set, [[\Yew\Framework\Web\User::getReturnUrl()]] will be used to obtain remembered URL.
     ...
     * @see remember()
     * @see \Yew\Framework\Web\User::getReturnUrl()
     */
     ...
     *
     * The canonical URL is constructed using the current controller's [[\Yew\Framework\Web\Controller::route]] and
     * [[\Yew\Framework\Web\Controller::actionParams]]. You may use the following code in the layout view to add a link tag
     * about canonical URL:
     *
     * ```php
