 ...
 *
 * I18N is configured as an application component in [[\Yew\FrameworkBase\Application]] by default.
 * You can access that instance via `Yew::$app->i18n`.
 *
...
class I18N extends Component
{
    ...
    public function format($message, $params, $language)
    {
        ...
        if (preg_match('~{\s*[\w.]+\s*,~u', $message)) {
            ...
            if ($result === false) {
                ...
                Yew::warning("Formatting message for language '$language' failed with error: $errorMessage. The message being formatted was: $message.", __METHOD__);
    ...
    public function getMessageFormatter()
    {
        if ($this->_messageFormatter === null) {
            ...
            $this->_messageFormatter = Yew::createObject($this->_messageFormatter);
        }
     ...
     * Can be given as array or string configuration that will be given to [[Yew::createObject]] to create an instance
    ...
    public function getMessageSource($category)
    {
        if (isset($this->translations[$category])) {
            ...
            return $this->translations[$category] = Yew::createObject($source);
        }
        ...
        foreach ($this->translations as $pattern => $source) {
            if (strpos($pattern, '*') > 0 && strpos($category, rtrim($pattern, '*')) === 0) {
                ...
                return $this->translations[$category] = $this->translations[$pattern] = Yew::createObject($source);
            }
        }
        ...
        if (isset($this->translations['*'])) {
            ...
            return $this->translations[$category] = $this->translations['*'] = Yew::createObject($source);
        }
