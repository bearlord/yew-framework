...
class GossipClusterState implements ClusterStateInterface
{
    ...
    public function tick(?int $now = null): array
    {
        ...
        // private $members ¡ª otherwise each worker would vote independently and cause
     ...
     * router rebuilds from the complete set ¡ª preventing a worker with an
     ...
     * this, so the table aggregates members learned by ANY worker ¡ª letting
            ...
            if ($table->exist($id)) {
                ...
                if ($existing !== false) {
                    ...
                    if ($existingHb > $localHb) {
                        // Shared row is newer ¡ª keep its status/host/port/hb.
